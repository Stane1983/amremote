...
int checkVer(int fd)
{
	...
	{
		fprintf(stderr, "failed to get version info: %s\n", strerror(errno));
		return FAIL;
	}
	/*check share data version info*/
	if (0 != strcasecmp(verBuf, SHARE_DATA_VERSION))
	{
		fprintf(stderr, "interface version not match: user[%s] unequal to kernel[%s]\n",
...
int setCfgPara(int devFd, const char *sysDir, S_CFG_FILE_T *cfgFile)
{
	...
	if (ioctl(devFd, REMOTE_IOC_SET_SW_DECODE_PARA, &cfgFile->sw_data) < 0)
	{
...
{
	if (ioctl(devFd, REMOTE_IOC_SET_KEY_NUMBER, &tabFile->tab.map_size) < 0 )
	{
		fprintf(stderr, "failed to set key number: %s\n", strerror(errno));
		return FAIL;
	}

	if (ioctl(devFd, REMOTE_IOC_SET_KEY_MAPPING_TAB, &tabFile->tab) < 0 )
	{
